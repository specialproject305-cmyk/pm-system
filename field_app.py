...
def field_app_page():
    ...
    overdue_count = len(ms_df[ms_df['planned_end'].dt.date < date.today()]) if total_tasks > 0 else 0
    ...
    with st.sidebar:
        ...
        total_tasks = len(ms_df)
        done_count = len(ms_df[ms_df['status'] == 'DONE'])
        overdue_count = len(ms_df[ms_df['planned_end'] < pd.Timestamp(date.today())]) if total_tasks > 0 else 0
    ...
    with tab4:
        ...
        with col_forecast1:
            ...
            if total_tasks > 0 and done_count < total_tasks:
                ...
                non_done_tasks = ms_df[ms_df['status'] != 'DONE']
                if not non_done_tasks.empty:
                    min_planned_end = non_done_tasks['planned_end'].min()
                    if pd.notna(min_planned_end):
                        days_until_deadline = (min_planned_end - pd.Timestamp(date.today())).days
    ...
    # ===== TAB 5: ISSUES =====
    with tab5:
        st.subheader("🔍 Issues & Blockers")
        st.info("No active blockers reported by you.")
