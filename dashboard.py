...
# ─────────────────────────────────────────────────────────────
# 🎨 DARK THEME & UI CSS (FIXED SIDEBAR VISIBILITY)
# ─────────────────────────────────────────────────────────────
def inject_dark_css():
    st.markdown("""
    <style>
        ...
        /* SIDEBAR STYLING - FIX TEXT VISIBILITY */
        ...
        .stSidebar h1, .stSidebar h2, .stSidebar h3, .stSidebar p, .stSidebar span, .stSidebar div {
            color: #FFFFFF !important;
        }
        .stSidebar .stSelectbox label, .stSidebar .stSelectbox div {
            color: #E2E8F0 !important;
        }
        .stSidebar button { 
            width: 100%; 
...
def create_stacked_bar(df, x_col, categories, colors):
    ...
    for i, cat in enumerate(categories):
        ...
        if cat in df.columns:
            ...
            # Add empty trace for missing category to maintain legend
            fig.add_trace(go.Bar(
                x=df[x_col] if len(df) > 0 else [], 
                y=[], 
                name=cat, 
                marker_color='#334155',
                opacity=0.3,
                hovertemplate=f"{cat}: 0<extra></extra>"
            ))
...
def dashboard_page():
    ...
    <div class='header-container'>
        ...
        <div style='display:flex; gap:15px; align-items:center;'>
             <button style='background:#334155; color:#FFFFFF !important; border:none; padding:8px 15px; border-radius:6px; cursor:pointer; font-size:0.8rem;'>
                 📅 Periode: 20 Mei - 20 Jun 2024
             </button>
             <button style='background:#38BDF8; color:#0F172A !important; border:none; padding:8px 15px; border-radius:6px; cursor:pointer; font-weight:bold; font-size:0.8rem;' onclick='document.location.reload()'>
                 🔄 Refresh
             </button>
        </div>
    </div>
        ...
        if not ms_df.empty:
            ...
            for _, row in step_df.iterrows():
                ...
                <div style='display:flex; align-items:center; margin-bottom:8px;'>
                    <div style='width:80px; font-size:0.8rem; color:#FFFFFF !important; font-weight:bold;'>{row['step']}</div>
    ...
    with col_m2:
        ...
        st.markdown(f"<div style='text-align:center; font-size:2rem; font-weight:bold; color:#FFFFFF !important;'>23</div><div style='text-align:center; color:#CBD5E1; font-size:0.8rem;'>TOTAL ISSUE</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
