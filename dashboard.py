...
def dashboard_page():
    ...
    with col_d1:
        ...
        st.markdown("<h3 style='color:#CBD5E1 !important; font-size:0.8rem; margin-bottom:5px; text-align:center;'>PROGRESS OVERVIEW</h3>", unsafe_allow_html=True)
        if 'status' in df.columns and not df.empty:
            status_counts = df['status'].value_counts()
            fig = px.pie(values=status_counts.values, names=status_counts.index, hole=0.6, 
                        color_discrete_map={'DONE':'#10B981','ON_TRACK':'#10B981','ONGOING':'#F59E0B','PENDING':'#94A3B8','DELAYED':'#EF4444','CRITICAL':'#7F1D1D'})
            fig.update_layout(height=220, margin=dict(t=5,b=5,l=5,r=5), paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#FFFFFF',size=9), showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col_d2:
        st.markdown("<div class='chart-box'>", unsafe_allow_html=True)
        st.markdown("<h3 style='color:#CBD5E1 !important; font-size:0.8rem; margin-bottom:5px; text-align:center;'>SITE BY KATEGORI</h3>", unsafe_allow_html=True)
        if 'site_category' in df.columns and not df.empty:
            ...
            fig2 = px.pie(values=cat_counts.values, names=cat_counts.index, hole=0.6,
                         color_discrete_map={'New Site':'#3B82F6','Collocation':'#8B5CF6','Upgrade':'#F59E0B','Relocation':'#EF4444'})
            fig2.update_layout(height=220, margin=dict(t=5,b=5,l=5,r=5), paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#FFFFFF',size=9), showlegend=False)
            st.plotly_chart(fig2, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col_d3:
        st.markdown("<div class='chart-box'>", unsafe_allow_html=True)
        st.markdown("<h3 style='color:#CBD5E1 !important; font-size:0.8rem; margin-bottom:5px; text-align:center;'>SITE STATUS BY VENDOR</h3>", unsafe_allow_html=True)
        if 'vendor' in df.columns and 'status' in df.columns:
            ...
            if not pivot.empty:
                ...
                fig3.update_layout(height=240, margin=dict(t=5,b=5,l=5,r=5), paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#FFFFFF',size=9), legend=dict(orientation='h', y=1.05, font=dict(size=8)))
