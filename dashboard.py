...
# ─────────────────────────────────────────────────────────────
# 🎨 PREMIUM EXECUTIVE MODERN CSS WITH HIGH-CONTRAST KPI CARDS
# ─────────────────────────────────────────────────────────────
def inject_premium_css():
    ...
    <style>
        ...
        /* 🛠️ FITUR UTAMA: REKAYASA ULANG KPI CARDS MENCOLOK & HIGH CONTRAST TEXT */
        .kpi-card { 
            border-radius: 12px; 
            padding: 18px 10px; 
            text-align: center; 
        ...
        /* Injeksi Gradasi Warna Solid Mencolok Per Jenis Data */
        .kpi-bg-total { background: linear-gradient(135deg, #1E3A8A 0%, #3B82F6 100%); }
        .kpi-bg-done { background: linear-gradient(135deg, #065F46 0%, #10B981 100%); }
        .kpi-bg-ongoing { background: linear-gradient(135deg, #92400E 0%, #F59E0B 100%); }
        .kpi-bg-pending { background: linear-gradient(135deg, #075985 0%, #0EA5E9 100%); }
        .kpi-bg-delayed { background: linear-gradient(135deg, #991B1B 0%, #EF4444 100%); }
        .kpi-bg-progress { background: linear-gradient(135deg, #374151 0%, #6B7280 100%); }

        /* Penyesuaian Teks Konten Dalam Kartu Agar Putih Murni Kontras */
        .kpi-icon { font-size: 1.6rem; margin-bottom: 3px; }
        .kpi-title { font-size: 0.72rem; color: rgba(255, 255, 255, 0.85); font-weight: 700; text-transform: uppercase; letter-spacing: 0.6px; }
        .kpi-value { font-size: 1.7rem; font-weight: 900; color: #FFFFFF !important; margin: 4px 0; text-shadow: 0 2px 4px rgba(0,0,0,0.2); }
        .kpi-subtitle { font-size: 0.68rem; color: rgba(255, 255, 255, 0.75); font-weight: 500; }
...
# ─────────────────────────────────────────────────────────────
# 📊 INTERACTIVE UTILITIES (DENGAN INJEKSI CLASS BG BARU)
# ─────────────────────────────────────────────────────────────
def get_safe_numeric(series):
    return pd.to_numeric(series, errors='coerce').fillna(0)

def render_kpi_card(icon, title, value, subtitle, bg_class):
    st.markdown(f"""
    <div class="kpi-card {bg_class}">
...
def dashboard_page():
    ...
    if 'region' not in df_projects.columns:
        df_projects['region'] = 'Jawa Barat'
    
    # ─── FORCE LIVE CLOCK & TIMEZONE WIB ───
    try:
    ...
    # 🧱 ENGINE 6 FILTER BERANTAI & HEADER BARU
    st.markdown(f"""
    ...
    with st.expander("🔍 PANEL KONTROL & FILTER UTAMA (Cascading Active)", expanded=True):
        ...
        # Sinkronisasi filter ke downstream table
    ...
    # RENDERING THE NEW COLOURED HIGH-CONTRAST CARDS GRID
    k_col1, k_col2, k_col3, k_col4, k_col5, k_col6 = st.columns(6)
    with k_col1: render_kpi_card("📡", "Total Sites", total_sites, "Portofolio Aktif", "kpi-bg-total")
    with k_col2: render_kpi_card("✅", "RFS / On Air", rfs_count, f"{rfs_count/total_sites*100:.1f}% Terbuka" if total_sites > 0 else "0%", "kpi-bg-done")
    with k_col3: render_kpi_card("⚙️", "In Progress", ongoing_count, "Sedang Eksekusi", "kpi-bg-ongoing")
    with k_col4: render_kpi_card("⏳", "Pending", pending_count, "Menunggu Antrean", "kpi-bg-pending")
    with k_col5: render_kpi_card("🔴", "Delayed", delayed_count, "Butuh Eskalasi", "kpi-bg-delayed")
    with k_col6: render_kpi_card("📊", "Avg Progress", f"{avg_progress:.1f}%", "Capaian Kumulatif", "kpi-bg-progress")
        ...
        with p_col2:
            ...
            if 'vendor' in df_projects.columns and not df_projects.empty:
                ...
                fig_vendor.update_layout(
                    barmode='stack',
                    height=280,
                    margin=dict(l=140, r=20, t=70, b=20),
                    showlegend=True,
                    legend=dict(
                        orientation="h",
                        yanchor="bottom",
        ...
        if not df_inventory.empty:
            ...
            with l_col2:
                ...
                if col_item_display in df_inventory.columns:
                    ...
                    fig_items = px.bar(
                        item_counts.sort_values(by='Volume', ascending=True),
                        x='Volume', 
                        y=col_item_display, 
                        orientation='h',
                        color='Volume', 
    ...
    <div style='text-align: center; color: #94A3B8; font-size: 0.75rem; padding: 10px 0;'>
        Sinkronisasi Terakhir Cloud Supabase: {waktu_sekarang} WIB | 🛠️ <b>System Enterprise Edition</b>
    </div>
    """, unsafe_allow_html=True)
