...
def create_material_comparison_chart(materials_df, theme):
    ...
    mat_disp = materials_df.head(10).copy()
    fig = go.Figure()
...
def dashboard_page():
    ...
    # FIX: Safe numeric conversion for progress
    avg_progress = 0
    if not filtered.empty and 'progress' in filtered.columns:
        try:
            progress_numeric = pd.to_numeric(filtered['progress'], errors='coerce')
            avg_progress = progress_numeric.mean()
            if pd.isna(avg_progress): avg_progress = 0
        except: avg_progress = 0
