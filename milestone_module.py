...
def milestone_page():
    ...
    with tab2:
        ...
        with st.form("add_ms"):
            ...
            with c5: mat_status = st.selectbox("Status Material", ["Belum Dicek","Lengkap","Tidak Lengkap"])
                ...
                for phase in phases:
                    for task in TOWER_TEMPLATE[phase]:
                        insert_row("milestones", {
                            ...
                            'weight': str(round(100/sum(len(TOWER_TEMPLATE[p]) for p in phases), 1)),
    ...
    with tab4:
        ...
        if not site_ms.empty:
            ...
            if sel:
                ...
                ridx = find_row_by_id("milestones", sel)
                with st.form("edit_ms"):
                    ename = st.text_input("Nama", value=ms.get('name',''))
                    estatus = st.selectbox("Status", ["PENDING","ONGOING","DONE","DELAYED"],
                                          index=["PENDING","ONGOING","DONE","DELAYED"].index(ms.get('status','PENDING')) if ms.get('status') in ["PENDING","ONGOING","DONE","DELAYED"] else 0)
                    emat = st.selectbox("Material", ["Belum Dicek","Lengkap","Tidak Lengkap"],
                                       index=["Belum Dicek","Lengkap","Tidak Lengkap"].index(ms.get('material_status','Belum Dicek')) if ms.get('material_status') in ["Belum Dicek","Lengkap","Tidak Lengkap"] else 0)
