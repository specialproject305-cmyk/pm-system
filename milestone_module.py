...
def milestone_page():
    ...
    # Sidebar Filter Site - PERTAHANKAN STRUKTUR ASLI
        ...
        if ms_df.empty:
            ...
            if is_all:
                ...
                if not site_ms.empty:
                    ...
                    # FIX: Fungsi terpisah untuk hindari lambda error
                    def check_critical(row):
                        try:
                            if pd.isnull(row["planned_end"]):
                                return False
                            end_date = row["planned_end"]
                            if hasattr(end_date, 'date'):
                                end_date = end_date.date()
                            return end_date < today and row["status"] != "DONE"
                        except:
                            return False
                    
                    site_ms["is_critical"] = site_ms.apply(check_critical, axis=1)
            ...
            with st.form("add_ms"):
                ...
                if st.form_submit_button("💾 Simpan"):
                    new_id = insert_row("milestones", {
                        ...
                        "name": name,
                        "planned_start": safe_date_string(ps),  # FIX: Pakai helper
                        "planned_end": safe_date_string(pe),
                        "weight": str(weight), 
                        "status": status
                    })
                    if new_id:  # FIX: Cek return value
    ...
    with tab3:
        if is_all: 
            ...
            if st.button("🚀 Generate Semua Task"):
                ...
                for phase in phases:
                    for task in TOWER_TEMPLATE[phase]:
                        new_id = insert_row("milestones", {
                            ...
                            "project_id": selected_site,
                            "name": f"[{phase}] {task}",
                            "planned_start": safe_date_string(curr),
                            "planned_end": safe_date_string(curr + timedelta(days=duration-1)),
                ...
                sync_milestone_to_site(selected_site)
                if success_count > 0:
                    st.success(f"Template Berhasil di-generate! ({success_count} tasks)")
                    st.rerun()
            ...
            if not site_ms.empty:
                ...
                with st.form("edit_form"):
                    ...
                    if st.form_submit_button("Update"):
                        success = update_row("milestones", sel_id, {"name": enama, "status": estatus})
                        if success:
                            sync_milestone_to_site(selected_site)
                            st.success("Updated!")
                            st.rerun()
                ...
                if st.button("🚀 Konfirmasi Import"):
                    ...
                    for _, r in df_import.iterrows():
                        new_id = insert_row("milestones", {
                            ...
                            "project_id": selected_site,
                            "name": str(r["name"]),
                            "planned_start": safe_date_string(r["planned_start"]),
                            "planned_end": safe_date_string(r["planned_end"]),
                            "weight": str(r["weight"]),
                            "status": str(r["status"])
                        })
                        if new_id:
                    ...
                    sync_milestone_to_site(selected_site)
                    if success_count > 0:
                        st.success(f"Berhasil mengimport {success_count} task!")
                        st.rerun()
